...
export default function UsersPage() {
  ...
  const deleteUser = async (userId: string) => {
    try {
      ...
      // User found, proceeding with deletion
      
      // Mark as deleted in usuarios collection (soft delete)
      const userRef = doc(db, 'usuarios', userId)
      // Marking user as deleted
      ...
      // User marked as deleted in database

      // Remove from local list
      setApprovedUsers(prev => prev.filter(u => u.id !== userId))
      // User removed from local list
      
      // User marked as deleted successfully
      // NOTE: User still exists in Firebase Authentication but was disabled in system
